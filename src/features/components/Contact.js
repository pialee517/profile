import { Alert, Box, Button, Card, CardContent, CardHeader, Grid, IconButton, TextField, Typography } from '@mui/material'
import React, { useRef, useState } from 'react'
import { Toaster, toast } from 'sonner';

import CancelPresentationIcon from '@mui/icons-material/CancelPresentation';
import {content, buttons} from './../../data/content'
    ...
    <Box ref={refContact}>
      ...
      <Grid item xs={style.grid.full} md={style.grid.half}>
        <Card elevation={0} 
          ...
          {items.map((item, idx)=>(
          <CardHeader key={`${item.title}}_${idx}`} 
            sx={{textAlign:'left',
